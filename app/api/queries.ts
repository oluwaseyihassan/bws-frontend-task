...
export const getPredictions = async (date: string, filterByPercentage: number = 0) => {
  const response = await api.get(`/prediction/${date}?include=predictions.type;participants;league.country;scores.type;state&filters=predictionTypes:237&filterByPercentage=${filterByPercentage}`);
